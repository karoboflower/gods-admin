...
import type { Table, TableColumn, TableExpose, TableProps, TableSetProps } from '@/components/Table';
...
export const useTable = (config: UseTableConfig) => {
  ...
  watch(
    () => currentPage.value,
    () => {
      skip.value = useSkip(currentPage.value, pageSize.value);
  ...
  const methods = {
    ...
    getList: async () => {
      ...
      try {
        ...
        const len = res?.list?.length || 0;

        if (len && len < pageSize.value && +skip.value === 0) {
          total.value = 0;
        }
        if (currentPage.value * pageSize.value < total.value) return;
        if (len && len === pageSize.value) {
          const newTotal = total.value + pageSize.value + 1;
          total.value = newTotal > 10000 ? 10000 : newTotal;
        }
      } catch (err) {
