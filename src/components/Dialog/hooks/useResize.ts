  ...
  const setupDrag = (elDialog: any, el: any) => {
    ...
    const handleMouseMove = (e: any) => {
      ...
      if (!isResizing) {
        ...
        } else if (offsetX > width - paddingRight && offsetY > paddingTop && offsetY < height - paddingBottom) {
          elDialog.style.cursor = 'ew-resize'; // 左右箭头
          currentResizeDirection = 'right';
        } else if (offsetY < paddingTop && offsetX > paddingLeft && offsetX < width - paddingRight) {
          elDialog.style.cursor = 'ns-resize'; // 上下箭头
          currentResizeDirection = 'top';
        } else if (offsetY > height - paddingBottom && offsetX > paddingLeft && offsetX < width - paddingRight) {
          elDialog.style.cursor = 'ns-resize'; // 上下箭头
